...
resource "aws_iam_role" "bedrock_kb" {
  name = "AmazonBedrockExecutionRoleForKnowledgeBase_nkbot"
...
resource "aws_iam_role_policy" "kb_model" {
  name = "AmazonBedrockFoundationModelPolicyForKnowledgeBase_nkbot"
...
resource "aws_iam_role_policy" "kb_oss" {
  name = "AmazonBedrockOSSPolicyForKnowledgeBase_nkbot"
  role = aws_iam_role.bedrock_kb.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "aws_iam_role_policy" "kb_s3" {
  name = "AmazonBedrockS3PolicyForKnowledgeBase_nkbot"
  role = aws_iam_role.bedrock_kb.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "aws_opensearchserverless_collection" "kb" {
  name = "bedrock-knowledge-base-nkbot"
  type = "VECTORSEARCH"
  depends_on = [
      ...
      Rules = [
        {
          ResourceType = "index"
          Resource = [
            "index/bedrock-knowledge-base-nkbot/*"
          ]
          Permission = [
            "aoss:UpdateIndex",
            "aoss:DescribeIndex",
        ...
        },
        {
          ...
          Resource = [
            "collection/bedrock-knowledge-base-nkbot"
          ]
          Permission = [
